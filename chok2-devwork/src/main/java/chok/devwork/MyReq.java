...
public class MyReq extends HttpServletRequestWrapper
{
	 ...
	 * 
	 * @param clazzName
	 *            request中子类值的值的key的前缀带“.”，父类则使用空字符串
	 * @param o
	 *            Object
	 */
			...
			for (int i = 0; i < descritors.length; i++)
			{
				try
				{
					...
					else
					{
						...
						if (obj == null)
						{
							obj = (Class.forName(descritors[i].getReadMethod().getReturnType().getName()))
									.newInstance();
							descritors[i].getWriteMethod().invoke(o, obj);
						}
						this.getFillObject(obj, pre_name + ".");
						// descritors[i].getWriteMethod().invoke(o, new
						// Object[]{request.getParameter(pre_name)});
					}
				}
				catch (Exception ex)
				{
	...
	public Map<String, Object> getDynamicSortParameterValueMap(Map<String, Object> param)
	{
		...
		for (Entry<String, Object> entry : param.entrySet())
		{
			...
			{
				if (k.indexOf("[sortName]") != -1)
					sortNameMap.put(k, v);
				if (k.indexOf("[sortOrder]") != -1)
					sortOrderMap.put(k, v);
			}
		}
		// 多列排序.排序
		List<Map.Entry<String, Object>> multiSortNameList = new ArrayList<Map.Entry<String, Object>>(
				sortNameMap.entrySet());
		...
		List<Map.Entry<String, Object>> multiSortOrderList = new ArrayList<Map.Entry<String, Object>>(
		...
		for (int i = 0; i < multiSortNameList.size(); i++)
		{
			...
			if (sortName.indexOf("m.") != -1)
				sortName = sortName.substring(2); // entity 为 Map时，去掉'm.'
		...
		if (multiSortList.size() > 0)
			param.put("multiSort", multiSortList);
		return param;
	}
	...
	{
		String Xip = getHeader("X-Real-IP");
		String XFor = getHeader("X-Forwarded-For");
		...
		{
			XFor = getHeader("Proxy-Client-IP");
		}
		if (StringUtils.isBlank(XFor) || "unknown".equalsIgnoreCase(XFor))
		{
			XFor = getHeader("WL-Proxy-Client-IP");
		}
		if (StringUtils.isBlank(XFor) || "unknown".equalsIgnoreCase(XFor))
		{
			XFor = getHeader("HTTP_CLIENT_IP");
		}
		if (StringUtils.isBlank(XFor) || "unknown".equalsIgnoreCase(XFor))
		{
			XFor = getHeader("HTTP_X_FORWARDED_FOR");
		}
		if (StringUtils.isBlank(XFor) || "unknown".equalsIgnoreCase(XFor))
		{
			XFor = getRemoteAddr();
